...
$near_deduper->add_key(field_number => 1,alias => \%nick_names);
$near_deduper->add_key(field_number => 2);
$near_deduper->dedupe_file("t/tab_delim.txt") or die $!;

open(UNIQ,"<t/tab_delim_uniqs.txt") or die $!;
my @lines = <UNIQ>;
close(UNIQ);
ok( $lines[0] eq  "Robert\tSmith\tWaverley\t100\n",'unique record 1 found' );
...
ok( $lines[0] eq  "Robert\tSmith\tWaverley\t101\n",'duplicate record 1 found' );
ok( $lines[1] eq  "Bob\tSmith\tWaverley\t102\n",   'duplicate record 2 found' );
